...
const ArticleDetail = () => {
  ...
  const fetchBlogs = async () => {
    ...
    try {
      ...
      if (data?.data) {
        const matchedPost = data?.data.find((p: BlogPost) => p.slug === slug);
        if (matchedPost) {
          matchedPost.content = DOMPurify.sanitize(matchedPost.content);
            ...
            <ol className="inline-flex items-center space-x-1 md:space-x-2">
              ...
              <li>
                <div className="flex items-center">
                  ...
                  <a href="#" className="ml-1 text-sm font-medium text-gray-500 md:ml-2">{category?.name}</a>
                </div>
              </li>
            </ol>
          </nav>
